...
<body>
    ...
    <form method = "post" action = "<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" class="register">
        ...
        <p>I already have an account. <a href="../PHP/login.php">Login</a></p>
    </form>
    </div>
    </div>
    ...
    <?php
        ...
        if(isset($_POST['submit'])){
            ...
            if($email_count>0){
                echo"<center><h4>Email already exists try login <button><a href='../PHP/login.php'>Log in</a></button><h4></center>";
            }
            else{
                if($password==$confirm){
                $insert_query ="insert into user (name, email, phone, password) values ('$name', '$email', '$phone', '$password')";
                $insert_query_result = mysqli_query($con,$insert_query) or die(mysqli_error($con));
                echo"<center>You have successfully registered,try login <button class='btn signup'><a href='../PHP/login.php'>Log in</a></button></center>";
                }
                else{
